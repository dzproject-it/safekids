...
export const container    = 'max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-4';
export const row          = 'flex flex-col sm:flex-row sm:flex-wrap items-start sm:items-center gap-3 sm:gap-4 justify-between';
export const catGroup     = 'flex items-center gap-2 overflow-x-auto pb-1 -mx-1 px-1';
...
export const sizeBtn      = (active: boolean) => `px-3 py-1.5 rounded-full text-xs font-medium transition-all whitespace-nowrap cursor-pointer ${active ? 'bg-white text-dark shadow-sm' : 'text-gray-500 hover:text-dark'}`;
